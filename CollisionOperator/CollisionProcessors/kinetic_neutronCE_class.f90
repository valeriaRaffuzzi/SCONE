module kinetic_neutronCEstd_class
  ...
  !!   }
  !!
  type, public, extends(collisionProcessor) :: kineticNeutronCEstd
    private
...
contains
  ...
  subroutine init(self, dict)
    class(kineticNeutronCEstd), intent(inout) :: self
    class(dictionary), intent(in)      :: dict
    character(100), parameter :: Here = 'init (kineticNeutronCEstd_class.f90)'

    ! Call superclass
    call init_super(self, dict)

    ! Read settings for kineticNeutronCEstd
  ...
  subroutine sampleCollision(self, p, collDat, thisCycle, nextCycle)
    ...
    real(defReal)                        :: r
    character(100),parameter :: Here = 'sampleCollision (kineticNeutronCEstd_class.f90)'
  ...
  subroutine implicit(self, p, collDat, thisCycle, nextCycle)
    ...
    real(defReal)                        :: wgt, w0, rand1, E_out, mu, phi, time
    real(defReal)                        :: sig_nufiss, sig_tot, k_eff
    real(defReal)                        :: lambda = ZERO
    character(100),parameter             :: Here = 'implicit (kineticNeutronCEstd_class.f90)'
    ...
    if ( self % nuc % isFissile()) then
      ...
      do i=1,n
        ...
        !prompt or delayed handling. thisCycle if delayed (precursorDungeon), nextCycle if prompt
  ...
  subroutine elastic(self, p, collDat, thisCycle, nextCycle)
    ...
    character(100),parameter :: Here = 'elastic (kineticNeutronCEstd_class.f90)'
  ...
  subroutine inelastic(self, p, collDat, thisCycle, nextCycle)
    ...
    class(particleDungeon),intent(inout)   :: nextCycle
    class(uncorrelatedReactionCE), pointer :: reac
    character(100),parameter  :: Here =' inelastic (kineticNeutronCEstd_class.f90)'
...
end module kinetic_neutronCEstd_class
