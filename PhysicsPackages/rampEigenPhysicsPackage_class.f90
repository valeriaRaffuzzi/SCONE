module rampEigenPhysicsPackage_class
  ...
  !!
  type, public,extends(physicsPackage) :: rampEigenPhysicsPackage
    private
    ! Building blocks
  ...
  end type rampEigenPhysicsPackage

contains

  subroutine run(self)
    class(rampEigenPhysicsPackage), intent(inout) :: self
    logical(defBool)                              :: ramping
    ...
    ramping = .true.
    call self % cycles(self % inactiveTally, self % inactiveAtch, self % N_inactive, ramping)
    ramping = .false.
    call self % cycles(self % activeTally, self % activeAtch, self % N_active, ramping)
  ...
  !!
  !!
  subroutine cycles(self, tally, tallyAtch, N_cycles, ramping)
    class(rampEigenPhysicsPackage), intent(inout) :: self
    ...
    do i = 1,nCyclesTot
      ...
      call self % nextCycle % normSize(normPop, pRNG)
      ...
      print *
      print *, 'Cycle: ', numToChar(i), ' of ', numToChar(nCyclesTot)
  ...
  subroutine generateInitialState(self)
    ...
    call self % initSource % generate(self % thisCycle, self % initialPop, self % pRNG)
    print *, "DONE!"
  ...
  subroutine init(self, dict)
    ...
    call dict % get( self % initialPop,'initialPop')
  ...
  subroutine printSettings(self)
    class(rampEigenPhysicsPackage), intent(in) :: self
...
end module rampEigenPhysicsPackage_class
