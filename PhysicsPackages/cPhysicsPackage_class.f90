...
contains
  ...
  subroutine cycles(self, tally, tallyAtch, N_cycles)
    ...
    integer(shortInt)                         :: i, n, Nstart, Nend, nParticles, nSource, j
    ...
    do i = 1, N_cycles
      ...
      gen: do n = 1, nParticles
        ...
        bufferLoop: do
          ...
          ! Transport particle until a collision
          call transOp % transport(neutron, tally, buffer, self % nextCycle)
          if (.not. neutron % isDead) then
            call collOp % collide(neutron, tally, buffer, self % nextCycle)
          end if

          if (.not. neutron % isDead) then
            !neutron % w = neutron % w / neutron % c_est
            nSource = int(abs( neutron % w / c_new ) + neutron % pRNG % get(), shortInt)
            neutron % w = ONE
              do j = 1, nSource
                call self % nextCycle % detain(neutron)
              end do
          end if
      ...
      call tallyAtch % getResult(res, 'c_est')
  ...
  subroutine init(self, dict)
    ...
    call dict % getOrDefault(self % c_0,'c_0', ONE)
    ...
    call locDict1 % store('c_est', locDict2)
    call locDict1 % store('display',['c_est'])
    ...
    call locDict2 % store('type','cImplicitClerk')
    call locDict1 % store('c_est', locDict2)
    call locDict1 % store('display',['c_est'])
